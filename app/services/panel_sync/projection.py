...
* **Дата окончания, статус, трафик, сквады и лимит устройств** берутся
  из панели при любом статусе аккаунта. Лимит трафика в этом форке остаётся
  биллинговым: shielded-измерения поднимают панельный лимит поверх оплаченной квоты.
...
Политики ``ROUTINE`` и ``BULK_SNAPSHOT`` не импортируют панельный лимит трафика
в оплаченную квоту. ``ADMIN_PULL`` явно принимает все поля панели.
``WEBHOOK`` тоже сохраняет биллинговый лимит: событие от записи shielded-лимита
не должно начислять трафик повторно. Подписку, намеренно отключённую в боте,
...
# Shielded usage raises the panel limit, not the purchased base quota.
ROUTINE = replace(PANEL_TRUTH, name='routine', takes_traffic_limit=False)
BULK_SNAPSHOT = replace(PANEL_TRUTH, name='bulk_snapshot', takes_traffic_limit=False)
...
def project_onto_subscription(
    ...
    *,
    ...
    # Только явный импорт принимает панельный лимит за оплаченную квоту.
    if (
    ...
    if policy.takes_squads and snapshot.squads:
        squads = merge_panel_squads(
            snapshot.squads,
            subscription.connected_squads,
            dimension_squad_policy.stripped_for(getattr(subscription, 'remnawave_id', None)),
        )
        if set(squads) != set(subscription.connected_squads or []):
