...
class RemnaWaveAPI:
    ...
    async def update_user(
        ...
        if active_internal_squads is not None:
            ...
            data['activeInternalSquads'] = dimension_squad_policy.filter_squads(panel_user_id, active_internal_squads)
    ...
    async def get_bandwidth_stats_user_inbounds(
        self, user_id: int, start_date: str, end_date: str, top_inbounds_limit: int = 100
    ) -> dict[str, Any]:
        ...
        # 3.0.0 адресует пользователя числовым id, как и соседний /bandwidth-stats/users/{id}.
        panel_user_id = coerce_panel_user_id(user_id)
        params = {'start': start_date, 'end': end_date, 'topInboundsLimit': top_inbounds_limit}
        response = await self._make_request(
            'GET', f'/api/bandwidth-stats/users/{panel_user_id}/inbounds', params=params
        )
        return response['response']
