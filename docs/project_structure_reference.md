...
- `.github/workflows/alembic-guard.yml` — файл
...
- `app/cabinet/schemas/subscription.py` — Python-модуль
  Классы: `ServerInfo`, `TrafficPurchaseInfo`, `SubscriptionData`, `SubscriptionStatusResponse`, `RenewalOptionResponse`, `RenewalRequest`, `TrafficPackageResponse`, `TrafficGrantResponse`, `TrafficPurchaseRequest`, `DevicePurchaseRequest`, `AutopayUpdateRequest`, `TrialActivateRequest`, `TrialInfoResponse`, `PurchaseSelectionRequest`, `PurchasePreviewRequest`, `TariffPurchaseRequest`
  Функции: нет
...
- `app/database/crud/ticket_notification.py` — Python-модуль
  ...
  Функции: нет
- `app/database/crud/traffic_dimension.py` — Python-модуль
  Классы: `TrafficDimensionKeyError`
  Функции: `list_traffic_dimensions`, `get_traffic_dimension`, `get_traffic_dimension_by_key`, `create_traffic_dimension` — Заводит измерение. Ключ неизменяем: на него ссылается леджер докупок., `update_traffic_dimension` — Точечно правит поля измерения. `key` менять нельзя., `set_dimension_inbounds`, `delete_traffic_dimension` — Удаляет измерение вместе с его состоянием у подписок и настройками тарифов., `count_dimension_subscriptions`
...
- `app/handlers/admin/traffic_dimensions.py` — Python-модуль
  Классы: нет
  Функции: `show_dimensions`, `show_dimension`, `toggle_dimension`, `cycle_enforcement`, `cycle_accounting`, `show_inbound_picker`, `toggle_inbound`, `start_limit_edit`, `start_title_edit`, `start_icon_edit`, `process_limit_edit`, `process_title_edit`, `process_icon_edit`, `start_create`, `process_create`, `confirm_delete`, `do_delete`, `register_handlers`
- `app/handlers/admin/traffic_why.py` — Python-модуль
  Классы: нет
  Функции: `traffic_why` — Разбор состояния измерений трафика конкретного пользователя., `register_handlers`
...
- `app/handlers/subscription/traffic.py` — Python-модуль
  Классы: нет
  Функции: `handle_add_traffic`, `handle_reset_traffic`, `confirm_reset_traffic`, `refresh_traffic_config`, `get_traffic_packages_info`, `select_traffic`, `add_traffic`, `add_traffic_package` — Покупка пакета трафика по идентификатору (`atp:{id}`)., `handle_no_traffic_packages`, `handle_switch_traffic`, `confirm_switch_traffic`, `execute_switch_traffic`
...
- `app/services/daily_subscription_service.py` — Python-модуль
  Классы: `DailySubscriptionService` (21 методов)
  Функции: нет
...
- `app/services/remnawave_service.py` — Python-модуль
  Классы: `RemnaWaveConfigurationError`, `RemnaWaveService` (64 методов)
  Функции: нет
...
- `app/services/traffic_dimension_enforcement.py` — Python-модуль
  Классы: `SquadTopology`, `BlockReason`, `EnforcementMode`, `StripPlan` (2 методов), `DimensionSquadPolicy` (7 методов), `EnforcementAction`, `EnforcementDecision` (1 методов), `BlastGuard` (1 методов)
  Функции: `resolve_mode`, `classify_squad` — К какому типу относится сквад с точки зрения измерения., `plan_squad_strip` — Разбирает сквады подписки на «снять», «нельзя снять» и «не знаем»., `panel_squads_for` — Что отправлять в панель: право подписки минус снятое., `merge_panel_squads` — Приводит `connected_squads` к панели, не теряя снятого блокировкой., `effective_panel_traffic_limit_bytes` — Лимит, который надо держать в панели с учётом «щитующих» измерений., `panel_key` — Числовой id панели из того, что реально доезжает до границы API., `decide` — Чистое решение по текущему состоянию измерения.
- `app/services/traffic_dimension_ledger.py` — Python-модуль
  Классы: `WindowUsage` (1 методов), `DimensionMeasurement` (1 методов), `LedgerCycleStats` (1 методов), `TrafficDimensionLedgerService` (10 методов), `TrafficDimensionLedgerScheduler` (6 методов)
  Функции: `window_start_for` — Первые UTC-сутки текущего расчётного окна измерения., `resolve_window_start` — `window_start_for`, вытаскивающий режим сброса из тарифа подписки., `squad_inbound_index` — `{squad_uuid: {inbound_uuid}}` из ответа `get_internal_squads()`., `reachable_inbounds` — Инбаунды, до которых подписка вообще может дотянуться., `applicable_specs` — Измерения, трафик по которым подписка физически может нагенерить., `sample_rows_from_matrix` — Ячейки матрицы, пригодные для записи в журнал., `store_samples` — Апсертит наблюдения по `bytes = GREATEST(existing, incoming)`., `window_usage` — Суммы по инбаундам за окно плюс первые сутки, которые журнал реально видел., `prune_samples` — Чистит наблюдения старше `keep_days`.
- `app/services/traffic_dimension_meter.py` — Python-модуль
  Классы: `InboundUsageMatrix` (4 методов), `InboundUsageReading` (1 методов)
  Функции: `parse_inbound_usage` — Разбирает ответ `/bandwidth-stats/users/{uuid}/inbounds` в матрицу., `resolve_displayed_used_gb` — Что показать пользователю: свежее измерение панели или кэш подписки., `panel_history_floor` — Самая ранняя дата, за которую панель ещё хранит пер-инбаунд историю.
- `app/services/traffic_dimension_notifier.py` — Python-модуль
  Классы: `TrafficDimensionNotifier` (10 методов)
  Функции: `install_notifier` — Связывает уведомителя с реконсилятором на старте бота.
- `app/services/traffic_dimension_reconciler.py` — Python-модуль
  Классы: `DimensionTransition`, `DimensionUsageSnapshot`, `ReconcileReport` (1 методов), `TrafficDimensionReconciler` (13 методов)
  Функции: `restore_policy_on_startup` — Поднимает карту блокировок из БД до того, как пойдут записи в панель., `reconcile_after_ledger` — Хук для планировщика журнала: пересчитали расход — сразу и решили.
- `app/services/traffic_dimensions.py` — Python-модуль
  Классы: `TrafficDimensionSpec` (5 методов), `DimensionState` (5 методов), `TrafficDimensionRegistry` (9 методов)
  Функции: `load_dimension_rows` — Строки состояния подписки, разложенные по id измерения., `get_dimension_states` — Состояния всех включённых измерений подписки, включая обычный трафик., `get_dimension_state`, `resolve_tariff_dimension_limit` — Сколько ГБ измерения включает тариф подписки., `ensure_dimension_row` — Возвращает строку состояния, создавая её при первом обращении., `load_tariff_dimension_config` — Настройки измерений тарифа, разложенные по id измерения., `sync_tariff_dimensions` — Приводит включённые объёмы измерений к текущему тарифу подписки., `dimension_keys`, `format_dimension_value` — Только цифры: «6.0 / 10 ГБ», с пометкой о блокировке или отсутствии данных., `format_dimension_usage` — Строка вида «⚪ WL Трафик (БС): 6.0 / 10 ГБ»., `format_extra_dimension_lines` — Готовые строки по всем измерениям, кроме обычного трафика., `grant_dimension_traffic` — Начисляет докупку измерения: запись в журнал покупок плюс пересчёт квоты., `sync_dimension_purchased_gb` — Пересобирает `purchased_gb` измерения по живым докупкам., `expire_dimension_purchases` — Приводит квоты всех измерений подписки к живым докупкам.
- `app/services/traffic_monitoring_service.py` — Python-модуль
  ...
  Функции: нет
- `app/services/traffic_package_service.py` — Python-модуль
  Классы: `AppliedGrant`, `PackageApplication` (4 методов)
  Функции: `apply_traffic_package` — Начисляет пакет подписке. Не коммитит — транзакцией владеет вызывающий.
- `app/services/traffic_packages.py` — Python-модуль
  Классы: `TrafficGrant` (2 методов), `TrafficPackage` (4 методов)
  Функции: `legacy_package_id` — `add_traffic_100` и дескриптор `gb100` — один и тот же пакет., `parse_packages` — Читает и старый, и новый формат., `legacy_projection` — Старый вид `{ГБ: цена}` для кода, который про измерения не знает., `package_by_id`, `split_price` — Разносит цену пакета по начислениям пропорционально гигабайтам., `build_package_id` — Устойчивый id из состава начислений., `parse_package_spec` — Разбирает админскую строку в дескрипторы пакетов., `format_package_spec` — Обратная операция: пакеты → строка, которую можно отредактировать., `describe_package` — Человекочитаемый состав пакета для админских экранов.
...
- `migrations/alembic/versions/0124_subscription_panel_identity_backfill.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/xb0001_add_wl_traffic_to_subscriptions.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/xb0002_traffic_dimensions.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/xb0003_traffic_dimension_samples_panel_id.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
...
- `tests/middlewares/`
- `tests/services/`
- `tests/test_alembic_revision_graph.py` — Python-модуль
  Классы: нет
  Функции: `test_no_duplicate_revision_ids` — Two files claiming the same revision id is the collision that broke prod., `test_every_down_revision_resolves`, `test_fork_migrations_stay_out_of_upstream_id_space` — Fork migrations must not use a bare numeric id upstream could also mint., `test_fork_branch_is_a_separate_head` — The fork branch must stay a head of its own., `test_runner_uses_plural_heads` — With a fork branch present, ``upgrade head`` (singular) raises MultipleHeads.
...
- `tests/database/crud/test_traffic_dimension.py` — Python-модуль
  Классы: нет
  Функции: `test_valid_keys`, `test_invalid_keys`
...
- `tests/external/test_remnawave_squad_filter.py` — Python-модуль
  Классы: `RecordingAPI` (4 методов)
  Функции: `clean_policy`, `test_blocked_squads_are_stripped_from_any_update`, `test_unblocked_users_pass_through_untouched`, `test_updates_that_do_not_touch_squads_stay_untouched` — `active_internal_squads=None` означает «сквады не трогай»., `test_restoring_all_squads_is_blocked_until_the_policy_is_cleared` — Ровно тот сценарий, ради которого фильтр и стоит на границе., `test_filter_is_case_insensitive`
...
- `tests/handlers/test_traffic_why.py` — Python-модуль
  Классы: нет
  Функции: `seed`, `test_breakdown_sums_the_day_and_shows_the_newest_first` — Сутки — одна строка: расход по инбаундам измерения складывается., `test_breakdown_reads_only_the_requested_panel_user` — Ключ журнала — числовой id: соседняя запись не должна попасть в разбор., `test_breakdown_without_a_panel_id_is_empty_not_an_error` — `remnawave_id IS NULL` — подписку ни разу не отдавали в панель., `test_dimension_without_inbounds_reads_nothing` — Измерение без инбаундов ничего не измеряет — запрос в БД не нужен.
...
- `tests/services/test_traffic_dimension_enforcement.py` — Python-модуль
  Классы: нет
  Функции: `make_state`, `test_mode_parsing`, `test_unknown_mode_never_enables_cutting_access` — Опечатка в настройке не должна молча включать снятие доступа., `test_squad_with_only_dimension_inbounds_is_pure`, `test_squad_without_dimension_inbounds_is_free`, `test_squad_with_both_kinds_is_mixed`, `test_squad_with_unknown_inbounds_is_free` — Про пустой сквад ничего не известно — снимать на догадке нельзя., `test_plan_selects_only_pure_squads`, `test_mixed_squad_refuses_the_whole_plan` — Снять смешанный сквад — отобрать оплаченный обычный доступ., `test_plan_records_squads_missing_from_the_panel_map`, `test_plan_is_case_insensitive`, `test_panel_squads_removes_stripped_and_keeps_order`, `test_panel_squads_without_stripping_is_identity`, `test_merge_restores_stripped_squads_into_entitlement` — Панель их не отдаёт — они сняты нами; право подписки не должно их терять., `test_merge_without_stripping_follows_the_panel` — Обычный случай: панель — источник правды, ничего не дописываем., `test_merge_deduplicates`, `test_policy_filters_only_known_users`, `test_policy_passes_none_through` — `active_internal_squads=None` означает «не трогай сквады»., `test_policy_without_a_panel_id_strips_nothing` — `remnawave_id IS NULL` — обычное состояние строки до привязки к панели., `test_policy_clear_restores_access`, `test_policy_set_with_empty_squads_clears`, `test_policy_replace_all_drops_previous_entries`, `test_policy_is_case_insensitive_about_squads`, `test_exhausted_quota_blocks`, `test_quota_within_limit_does_nothing`, `test_unlimited_dimension_is_never_blocked`, `test_unlimited_dimension_releases_an_existing_block` — Админ снял лимит — доступ обязан вернуться., `test_fresh_window_unblocks`, `test_already_blocked_and_still_exhausted_is_a_noop` — Иначе уведомление уходило бы каждый цикл., `test_unknown_usage_never_blocks`, `test_unknown_usage_holds_an_existing_block` — Молчащая панель не повод выдать квоту заново., `test_coverage_gap_holds_an_existing_block` — Журнал не покрывает начало окна — цифре нельзя верить в обе стороны., `test_coverage_gap_does_not_block_a_free_subscription`, `test_mixed_topology_refuses_instead_of_blocking`, `test_notify_only_dimension_blocks_without_a_plan` — enforcement=notify_only: состояние фиксируем, сквады не трогаем., `test_blast_guard_allows_normal_volume`, `test_blast_guard_trips_on_absolute_count`, `test_blast_guard_trips_on_share` — Мелкая база: 30 из 100 — это поломка, а не массовое исчерпание квоты., `test_blast_guard_thresholds_are_individually_disableable`, `test_blast_guard_ignores_share_without_scanned`, `test_subquota_leaves_the_panel_limit_alone` — Поведение «как панель»: трафик измерения расходует и основную квоту., `test_shielded_raises_the_limit_by_the_dimension_usage`, `test_shield_rounds_up_to_whole_gigabytes` — Гистерезис: лимит переставляется не чаще раза на гигабайт., `test_unknown_usage_never_inflates_the_limit` — Иначе молчащая панель раздала бы лишнюю квоту на выдуманную величину., `test_unlimited_base_stays_unlimited`, `test_shields_from_several_dimensions_add_up`, `test_near_limit_selects_only_actionable_states` — Уведомителю отдаём только то, по чему он может принять решение.
- `tests/services/test_traffic_dimension_ledger.py` — Python-модуль
  Классы: `FakeInbound`, `FakeSquad`
  Функции: `make_spec`, `test_window_start_matches_reset_strategy`, `test_window_never_predates_subscription` — Трафик до появления подписки — не её трафик., `test_month_rolling_short_month_uses_last_day` — 31-е число в феврале — последний день месяца, а не исключение., `test_panel_last_reset_wins_for_month_rolling` — Если панель сказала, когда сбросила, спорить не с чем., `test_squad_index_lowercases_everything`, `test_squad_index_skips_squads_without_uuid`, `test_reachable_inbounds_unions_connected_squads`, `test_reachable_inbounds_ignores_unknown_squad`, `test_reachable_inbounds_handles_empty_input`, `test_applicable_specs_needs_overlap` — Нет инбаунда измерения в сквадах — трафик по нему физически невозможен., `test_sample_rows_keeps_only_wanted_inbounds`, `test_sample_rows_drop_zero_cells`, `test_sample_rows_refuse_matrix_without_daily_series` — Свёрнутая в один день матрица сломала бы GREATEST-апсерт навсегда., `test_sample_rows_need_a_panel_id` — Подписка без `remnawave_id` в журнал не пишется: ключа у наблюдения нет., `test_window_usage_sums_requested_inbounds_only`, `test_measurement_reports_coverage_gap`, `test_unknown_measurement_is_not_a_coverage_gap` — `known=False` — отдельная категория: о покрытии речи вообще нет.
- `tests/services/test_traffic_dimension_ledger_db.py` — Python-модуль
  Классы: `FakeReading` (2 методов), `FakePanelReading` (2 методов)
  Функции: `sample`, `test_upsert_keeps_the_larger_value` — Панель отдала меньше, чем уже записано, — значит, был TRUNCATE, а не откат., `test_window_usage_respects_boundaries`, `test_window_usage_isolates_users`, `test_window_usage_without_samples_is_uncovered` — Пустой журнал — это дыра в покрытии, а не подтверждённый ноль., `test_prune_drops_only_old_samples`, `test_prune_disabled_by_zero`, `make_subscription`, `make_dimension`, `test_state_is_written_from_the_ledger`, `test_failed_reading_keeps_previous_value` — Панель молчит — прежняя цифра лучше нуля: ноль раздал бы квоту всем разом., `test_window_rollover_resets_the_counter` — Новое окно — новый счёт: наблюдения прошлого окна в сумму не попадают., `test_legacy_panel_counter_never_goes_backwards` — Панель без посуточных рядов отдаёт только текущую неделю — счётчик держим., `test_refresh_subscription_end_to_end` — Снять с панели → записать в журнал → пересчитать состояние., `test_refresh_skips_subscription_without_dimension_squads` — Нет инбаунда измерения в сквадах — панель не дёргаем вовсе.
- `tests/services/test_traffic_dimension_meter.py` — Python-модуль
  Классы: `TestResolveDisplayedUsedGb` (5 методов)
  Функции: `test_daily_matrix_sums_to_top_inbound_totals` — Опора всей модели: посуточная разбивка сходится с итогами панели., `test_matrix_keeps_per_day_breakdown`, `test_only_requested_inbounds_are_summed`, `test_falls_back_to_top_inbounds_when_panel_has_no_series` — Старая панель без рядов: суммы верны, посуточная разбивка — нет., `test_empty_payload_is_zero_not_crash`, `test_series_longer_than_categories_is_clipped` — Сетка дат — источник истины: лишние точки ряда игнорируются., `test_panel_history_floor_is_current_panel_week`, `test_panel_history_floor_can_be_disabled`
- `tests/services/test_traffic_dimension_notifier.py` — Python-модуль
  Классы: `Harness` (2 методов)
  Функции: `make_user`, `make_transition`, `make_snapshot`, `make_report`, `test_observe_sends_nothing` — Наблюдение обязано быть беззвучным — иначе это уже не наблюдение., `test_notify_mode_does_not_claim_access_was_cut` — В notify доступ ещё открыт: обещать обратное нельзя., `test_enforce_mode_reports_the_block`, `test_rolled_back_block_sends_nothing` — Панель не приняла запись — доступ открыт, сообщать не о чем., `test_unblock_is_reported`, `test_hold_and_refuse_are_not_user_facing` — Пользователь не может ни починить топологию, ни оживить панель., `test_warning_respects_the_user_threshold`, `test_warning_respects_the_user_opt_out`, `test_block_ignores_the_warning_opt_out` — Отключение подсказок не отключает сообщение о закрытом доступе., `test_warning_is_sent_once_per_window`, `test_new_window_rearms_the_warning` — Ключ включает начало окна: новый период — новое предупреждение., `test_message_carries_the_admin_authored_dimension_title` — Заголовки измерений — строки из БД, а не ключи локализации., `test_message_follows_the_user_language`, `test_unknown_user_is_skipped_without_crashing`, `test_every_dimension_event_has_an_email_template` — Пользователь без Telegram обязан узнать о закрытом доступе тоже., `test_email_copy_comes_from_the_same_locale_key` — Одна формулировка на оба канала: иначе правку внесут в одном месте.
- `tests/services/test_traffic_dimension_reconciler.py` — Python-модуль
  Классы: `FakeApi` (2 методов)
  Функции: `clean_policy` — Карта блокировок живёт в процессе — тесты не должны течь друг в друга., `seed`, `block_change`, `state_row`, `state_rows`, `test_observe_leaves_no_trace` — Режим наблюдения обязан быть полностью безопасным., `test_notify_records_without_touching_the_panel`, `test_enforce_strips_only_the_dimension_squad`, `test_entitlement_is_not_rewritten_by_a_block` — `connected_squads` — право подписки, блокировка его не трогает., `test_failed_panel_write_does_not_pretend_access_is_closed` — Иначе в БД стоит блокировка, а пользователь ходит — и никто не заметит., `test_blast_guard_suppresses_the_block`, `test_unblock_restores_the_full_entitlement`, `test_failed_unblock_keeps_the_filter` — Панель не подтвердила возврат — фильтр обязан остаться, иначе доступ утечёт., `test_hold_is_reported_once_not_every_cycle` — Иначе уведомление о «нет данных» уходило бы раз в три часа., `test_mixed_squad_refusal_writes_no_panel_call`, `test_publish_policy_rebuilds_the_map_from_the_database` — После рестарта карта обязана подняться из БД до первой записи в панель., `test_publish_policy_ignores_unblocked_rows`
- `tests/services/test_traffic_dimensions.py` — Python-модуль
  Классы: `TestSpec` (7 методов), `TestBaseState` (2 методов), `TestRowState` (9 методов), `TestFormatting` (5 методов)
  Функции: `test_accounting_mode_resolution_defaults_to_subquota` — Незнакомое значение не должно молча включать компенсацию лимита в панели.
...
- `tests/services/test_traffic_package_apply.py` — Python-модуль
  Классы: нет
  Функции: `seed`, `dimension_row`, `purchases`, `test_dimension_only_package_leaves_base_traffic_alone` — WL-пакет не имеет права трогать обычный лимит., `test_base_only_package_uses_the_legacy_path`, `test_mixed_package_grants_both_and_splits_the_price`, `test_grant_for_unknown_dimension_is_skipped_not_fatal` — Измерение могли удалить после настройки пакета — остальное честно., `test_unlimited_base_grant_clears_base_purchases_only` — Переход на безлимит не должен сжигать оплаченные пакеты измерений., `test_dimension_purchase_does_not_inflate_base_limit` — Ровно та ошибка, ради которой в таблице появился дискриминатор., `test_expired_dimension_purchase_does_not_drop_base_limit`, `test_dimension_quota_is_base_plus_purchased`, `test_purchases_accumulate`, `test_purchased_gb_is_recomputed_not_decremented` — Пересчёт от источника сходится к правде даже из рассинхрона., `set_tariff_included` — Заводит тариф и связывает его с подпиской., `test_new_row_takes_the_included_volume_from_the_tariff` — Ровно то, ради чего заводился tariff_traffic_dimensions., `test_tariff_without_the_dimension_falls_back_to_its_default` — Тариф измерение не включает — берём умолчание самого измерения., `test_zero_included_means_unlimited`, `test_sync_propagates_a_changed_tariff_setting` — Правка тарифа обязана доходить до уже существующих подписок., `test_sync_keeps_paid_topups` — Смена тарифа не сжигает оплаченные докупки — у них свой срок., `test_sync_is_idempotent`
- `tests/services/test_traffic_packages.py` — Python-модуль
  Классы: нет
  Функции: `test_legacy_mapping_is_read_unchanged` — Тарифы, настроенные до появления измерений, обязаны работать как были., `test_legacy_mapping_skips_garbage_entries`, `test_descriptor_list_is_parsed`, `test_wl_only_package_is_valid` — Один из трёх заявленных видов пакета — только трафик измерения., `test_unusable_descriptors_are_dropped` — Пакет, который нельзя купить, не должен доезжать до кнопки., `test_duplicate_ids_keep_the_first`, `test_grant_dimension_is_normalised`, `test_empty_input_is_empty_output`, `test_projection_keeps_old_readers_working` — Клавиатуры, кабинет и автопокупка читают именно этот словарь., `test_projection_skips_disabled_packages`, `test_legacy_round_trip` — Старый словарь → пакеты → тот же словарь., `test_legacy_callback_id_matches_the_synthesised_package`, `test_package_by_id_returns_none_for_unknown`, `test_single_grant_takes_the_whole_price`, `test_price_is_split_by_gigabytes`, `test_shares_always_sum_to_the_package_price` — Иначе отчёты по измерениям расходятся с суммой транзакции., `test_remainder_goes_to_the_first_grant`, `test_zero_gb_grants_split_evenly` — Безлимитные начисления не имеют пропорции — делим поровну., `test_no_grants_is_no_shares`, `test_free_package_splits_to_zeros`, `test_unlimited_base_grant_is_recognised`, `test_grant_lookup_by_dimension`, `test_legacy_admin_syntax_is_unchanged` — Строки, введённые до появления измерений, обязаны значить то же самое., `test_dimension_only_syntax`, `test_mixed_syntax`, `test_all_three_shapes_in_one_line` — Ровно те три вида пакета, ради которых всё затевалось., `test_ids_are_stable_across_edits` — id уходит в callback и корзину: пересохранение не должно его менять., `test_spec_round_trip`, `test_unparseable_specs_yield_nothing`, `test_separators_are_forgiving`, `test_duplicate_compositions_keep_the_first`, `test_dimension_key_case_is_normalised`, `test_describe_uses_admin_titles`, `test_describe_marks_unlimited_base`
