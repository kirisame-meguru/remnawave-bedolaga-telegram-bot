    ...
    *[
        {
            ...
            'description': {
                'ru': 'Уведомление о квоте отдельного направления трафика',
                'en': 'Notification about a traffic dimension quota',
                'zh': '独立流量维度配额通知',
                'ua': 'Сповіщення про квоту окремого напрямку трафіку',
            },
            'context_vars': ['dimension', 'used', 'limit', 'percent'],
        }
        for type_value in EmailNotificationTemplates._TRAFFIC_DIMENSION_EMAIL_TONE
    ],
]
...
SAMPLE_CONTEXTS: dict[str, dict[str, Any]] = {
    ...
    **{
        type_value: {'dimension': 'WL', 'used': 9.0, 'limit': 10, 'percent': 90.0}
        for type_value in EmailNotificationTemplates._TRAFFIC_DIMENSION_EMAIL_TONE
    },
}
...
class _TemplatePlaceholder(str):
    __slots__ = ()
...
def _placeholder_context(notification_type: str) -> dict[str, Any]:
    ...
    if not type_meta:
        return {}
    return {
        var: _TemplatePlaceholder(f'{{{var}}}')
