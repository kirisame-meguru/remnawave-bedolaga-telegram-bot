...
def test_automatic_pulls_do_not_capitalize_shielded_traffic():
    from app.services.panel_sync import ROUTINE

    for policy in (ROUTINE, BULK_SNAPSHOT, WEBHOOK):
        subscription = _sub(traffic_limit_gb=100)
        for _ in range(2):
            project_onto_subscription(
                subscription,
                PanelSnapshot(status='ACTIVE', traffic_limit_gb=120, traffic_used_gb=20),
                now=NOW,
                policy=policy,
            )
        assert subscription.traffic_limit_gb == 100
        assert subscription.traffic_used_gb == 20
...
def test_dimension_block_does_not_erase_entitlement(monkeypatch):
    from app.services.panel_sync import projection
    from app.services.traffic_dimension_enforcement import DimensionSquadPolicy

    squad_policy = DimensionSquadPolicy()
    squad_policy.set_for(42, ['blocked-squad'])
    monkeypatch.setattr(projection, 'dimension_squad_policy', squad_policy, raising=False)
    subscription = _sub(remnawave_id=42, connected_squads=['old-squad', 'blocked-squad'])

    project_onto_subscription(subscription, PanelSnapshot(status='ACTIVE', squads=('new-squad',)), now=NOW)

    assert subscription.connected_squads == ['new-squad', 'blocked-squad']
